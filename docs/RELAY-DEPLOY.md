...
**ระหว่างที่ยังไม่เปิด token ของเครื่องแต่ละเครื่องเปิด `/w/{id ของตัวเอง}` ได้ด้วย** — เจ้าของเครื่องทุกคนถือ token
ที่เรียกอุโมงค์ของเครื่องตัวเองได้ สิ่งที่เข้าถึงได้คือ ComfyUI ในเครื่องของเขาเอง (เครื่องรุ่นใหม่ยังจำกัดให้เห็นแค่งานที่มาทาง
อุโมงค์ ดู "สิ่งที่เครื่องตอบเอง") ที่เหลือคือใช้ทรัพยากรของ relay ซึ่ง relay จำกัดไว้ต่อ worker แล้ว
(`SessionRequestBufferBytes` `RequestBufferBudgetBytes` `SendStallSeconds` — เครื่องที่หยุดอ่าน socket ถูกตัด session
ไม่ได้ถือบัฟเฟอร์ของทั้ง relay ไว้) — แต่ยังควรเปิดข้อ 3 ให้เร็วที่สุดหลัง xmanstudio พร้อม
...
รายการนี้บอกได้แค่ว่า **เส้นทางไหน** ไม่รู้ว่าเป็นงานของใคร — relay ไม่เห็น ComfyUI ของเจ้าของ
การจำกัดให้แตะได้แค่งานของลูกค้าจึงอยู่ที่ตัวเครื่อง (หัวข้อถัดไป) เครื่อง v0.1 ที่ยังไม่อัปเดตได้แค่รายการนี้ชั้นเดียว
...
- **เห็นแค่งานที่มาทางอุโมงค์** — `GET /history` ทั้งรายการได้ 403 · `GET /history/{id}` ของ prompt ที่ไม่ได้มาทางอุโมงค์ได้ `{}`
  ...
  เรนเดอร์ และส่ง `prompt_id` ไปด้วย · `POST /upload/image` รับเฉพาะไฟล์ชื่อ `aixman-*` ลงชั้นบนสุดของโฟลเดอร์ input
  (`type`/`subfolder` อื่นได้ 403 `upload-not-allowed`) — ทั้งหมดนี้คือวิธีที่ aixman ใช้อยู่แล้ว
- **เครื่องตั้ง `prompt_id` เอง** — `prompt_id` `number` `front` ที่ผู้เรียกส่งมาถูกทิ้ง (เดิมผู้ถือ token ส่ง id ของงานเจ้าของ
  มาได้ แล้วสั่ง purge ลบรูปของเจ้าของ) ComfyUI รุ่นที่ยังไม่รับ id จากผู้ส่งก็ใช้ได้ — เครื่องใช้ id ที่ ComfyUI ตอบ
- **aixman วางสายระหว่างส่งงาน** — เครื่องอ่านคำตอบของ ComfyUI จนจบเองแล้วบันทึกงานไว้ ไม่ปล่อยให้เรนเดอร์โดยไม่มีใครติดตาม
  ถ้าคำตอบไม่มาเลย เครื่องถาม queue/history ของ ComfyUI ว่ารับไปแล้วหรือยัง
...
- **`POST /aixman/purge {prompt_id}`** — ลบไฟล์ผลงาน ไฟล์ที่ aixman อัปโหลดให้งานนั้น และประวัติใน ComfyUI
  ...
  หาโฟลเดอร์ของ ComfyUI จาก ComfyUI เอง (อ่านใหม่ทุกนาที — เจ้าของอาจปิด ComfyUI ตัวหนึ่งแล้วเปิดอีกตัวที่พอร์ตเดิม)
  ถ้าหาไม่เจอให้ตั้ง `ComfyBaseDirectory` · ไฟล์ที่เขียนก่อนงานเริ่มไม่ถูกลบแม้ชื่อตรง (ชื่อแบบ `ComfyUI_00012_.png`
  ซ้ำกันข้ามโฟลเดอร์ได้) · id ที่ยังอยู่ในคิวของ ComfyUI ได้ 503 `prompt-running` แม้ history จะบอกว่าเสร็จ
- **กด STOP = ส่งงานให้ครบก่อนหยุด** — ไม่รับงานใหม่ (`stage:"draining"`) แต่ยังต่อ relay ไว้
  จนงานที่รับไว้เรนเดอร์เสร็จ และ aixman purge **งานที่เพิ่งเสร็จนั้นเอง** หรือผ่านไป 90 วินาทีสำหรับ aixman ที่ยังไม่เรียก purge
  (นานสุด 45 นาที) — purge งานเก่า หรือ purge อัตโนมัติของเครื่องเอง ไม่นับว่าเก็บผลงานแล้ว
  · มีปุ่ม "หยุดทันที" แยกต่างหาก พร้อมบอกว่างานนั้นจะไม่ได้ค่าตอบแทน
  การติดตั้งอัปเดตก็ส่งงานให้ครบก่อนเหมือนกัน แล้วกลับมาแชร์เองหลังอัปเดต · ออกจากโปรแกรมจาก tray ถามแบบเดียวกัน
  เมื่อยังมีผลงานที่ aixman ยังไม่ได้เก็บ (ไม่ใช่แค่ตอนกำลังเรนเดอร์)
- **จับคู่ใหม่ระหว่างแชร์** — ตั้งแต่ก่อนส่งรหัสจนต่อด้วยตัวตนใหม่ เครื่องไม่รับงาน (`stage:"draining"`) และหลังได้ตัวตนใหม่
  รอให้ aixman เก็บผลงานที่ค้างของตัวตนเดิมก่อน (นานสุด 2 นาที) ถ้ากำลังเรนเดอร์อยู่ถูกปฏิเสธก่อนใช้รหัสเหมือนเดิม
...
งานอ่านแบบเคอร์เซอร์: เครื่องส่ง `updated_after` = `updated_at` ใหม่สุดที่เคยเห็น (จำไว้ใน ledger แยกตาม worker id —
จับคู่ใหม่ = เริ่มนับใหม่) แล้ว xmanstudio ตอบงานที่เปลี่ยนหลังเวลานั้น เก่าไปใหม่ ทีละ 200 พร้อม `jobs_more` ถ้ายังมีต่อ
เครื่องอ่านในรอบเดียวกันได้สูงสุด 2 หน้า ที่เหลืออ่านต่อในอีก 1 นาที (โควตา xmanstudio 10 ครั้ง/นาที/เครื่อง — กดรีเฟรชถี่แค่ไหนก็ไม่เกิน) ·
worker ที่ยังไม่มีเคอร์เซอร์เริ่มที่ ตอนนี้ − (ระยะพัก + 48 ชม.) · xmanstudio รุ่นก่อนเคอร์เซอร์ไม่สนใจ `updated_after`
(คำตอบไม่มี `jobs_more`) — เครื่องใช้ 50 งานล่าสุดแบบเดิมและไม่จำเคอร์เซอร์ · ถ้า xmanstudio ตอบ 422 ที่ `updated_after`
เครื่องถามซ้ำแบบเดิมทันที ไม่ขึ้นแดงว่าตัวตนถูกปฏิเสธ
...
| `RequestBufferBudgetBytes` | 192 MiB | ส่วนที่ body ของ request (อัปโหลด) ใช้ได้รวมกันทั้ง relay — ที่เหลือเป็นของคำตอบเสมอ probe กับไฟล์ผลงานของเครื่องอื่นจึงไม่ติดหลังอัปโหลดที่ค้าง |
| `SessionRequestBufferBytes` | 64 MiB | body ของ request ที่ถือไว้ให้เครื่องเดียว (ไม่ต่ำกว่า `MaxRequestBodyBytes`) — เต็มแล้วเครื่องนั้นได้ 503 `relay-busy` เครื่องอื่นไม่เกี่ยว |
| `SendStallSeconds` | 30 | เขียนลง socket ของเครื่องหนึ่งช่วง (1 MiB) ได้นานเท่านี้ เกินแล้วถือว่าเครื่องหยุดอ่าน ตัด session ทิ้ง — เดิมเครื่องที่ส่ง heartbeat แต่ไม่อ่านถือบัฟเฟอร์ไว้ได้ไม่จำกัด |
...
**ไคลเอนต์ต้องมาหลัง aixman เท่านั้น — ข้อนี้บังคับ ไม่ใช่คำแนะนำ** ไคลเอนต์รุ่นนี้ปฏิเสธ `POST /prompt` ด้วย
503 `{stage}` หรือ 409 `busy` ทุกครั้งที่เจ้าของอยู่หน้าเครื่อง มีโปรแกรมเต็มจอ อยู่นอกตารางเวลา หรือ ComfyUI ของเจ้าของเองมีคิว
(v0.1 กั้นแค่ `/aixman/ready` แล้วส่ง `/prompt` ต่อเสมอ) aixman รุ่นที่รันอยู่บน `main` นับ 503 นั้นเป็นการลองที่ล้ม
("ComfyUI rejected the workflow (HTTP 503)") และไม่ probe เครื่องที่สถานะ `ready` ซ้ำ รอบถัดไป 10 วินาทีก็เลือกเครื่องเดิมอีก
งานมี `maxAttempts=2` — **ทุกงานที่ส่งไปเครื่องที่พักหรือไม่ว่างจะล้มและคืนเครดิตภายในราว 10 วินาที**
aixman `feat/sharing-complete` (สัญญา C5: stage-503 = คืนเข้าคิวโดยไม่นับครั้ง แล้วตั้งเครื่องเป็น `warming`) ต้อง live บน prod ก่อน
และไคลเอนต์ถอยกลับเร็ว ๆ ไม่ได้: แท็กหนึ่งแท็ก Velopack ติดตั้งให้ทุกเครื่องเองภายในรอบตรวจอัปเดต
...
ไคลเอนต์
- [ ] C0 **ก่อนแท็ก:** aixman `feat/sharing-complete` live บน prod แล้ว — ตรวจว่างานที่ส่งไปเครื่องที่ตอบ 503 `{stage}`
      กลับเข้าคิวโดยไม่นับครั้ง (ไม่ใช่ "ComfyUI rejected the workflow (HTTP 503)") ถ้ายังไม่ใช่ **ห้ามแท็ก** — ดูเหตุผลใต้ "ลำดับ deploy"
      ตรวจแล้วตั้ง repository variable `AIXMAN_C5_LIVE=true` (Settings → Secrets and variables → Actions → Variables)
      release workflow ไม่ยอม build จนกว่าจะตั้ง
- [ ] C1 ปล่อยรุ่นใหม่ด้วยแท็ก — release workflow รันเทสต์ relay และโหนดก่อน publish เทสต์แดง = ไม่ปล่อย
      (แท็กเมื่อไรเครื่องทุกเครื่องอัปเดตเอง เรียกคืนไม่ได้ — ทำ C0 ก่อนทุกครั้ง)
